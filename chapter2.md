...
* 文档内容，它是一些在它们之间具有链接的对象
  形成有向图。这些对象描述了文档的结构（页面，元数据，字体和资源）。
* 页面内容，在一个页面上使用一系列用于放置文本和图形的运算符进行描述。
* 文件结构，包括header(文件头)，trailer(文件尾)和交叉引用表，帮助程序找到并读取文件的内容。
...
* stream(流)，由字典和一些二进制数据组成。这些习惯了存储PDF图形运算符的流，以及其他二进制数据，如图像和字体。
